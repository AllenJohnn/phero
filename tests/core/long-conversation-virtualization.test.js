...
import { deduplicateMessages, reindexMessages } from '@/core/capture/deduplication.js';
describe('Long Virtualized Conversation Simulation (100+ turns)', () => {
  ...
  it('correctly merges multiple backward sliding virtual windows into a full chronological 1..100 conversation', () => {
    ...
    expect(reindexed[0].content[0].text).toBe('Message content turn 1');
    expect(reindexed[99].content[0].text).toBe('Message content turn 100');
  });
});