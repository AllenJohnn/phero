...
describe('Full Six-Way End-to-End Transfer Suite', () => {
  ...
  const getSourceDoc = provider => {
    if (provider === 'chatgpt') {
      const html = loadFixture('chatgpt-sample.html');
      return new JSDOM(html, {
        url: 'https://chatgpt.com/c/test-chatgpt'
      }).window.document;
    }
    if (provider === 'claude') {
      const html = loadFixture('claude-sample.html');
      return new JSDOM(html, {
        url: 'https://claude.ai/chat/test-claude'
      }).window.document;
    }
    // gemini
    const html = loadFixture('gemini-sample.html');
    return new JSDOM(html, {
      url: 'https://gemini.google.com/app/test-gemini'
    }).window.document;
  };
  const getDestinationDoc = provider => {
    if (provider === 'chatgpt') {
      return new JSDOM('<!DOCTYPE html><html><body><div id="prompt-textarea" contenteditable="true"></div></body></html>', {
        url: 'https://chatgpt.com/'
      }).window.document;
    }
    if (provider === 'claude') {
      return new JSDOM('<!DOCTYPE html><html><body><div class="ProseMirror" contenteditable="true"><p><br></p></div></body></html>', {
        url: 'https://claude.ai/new'
      }).window.document;
    }
    // gemini
    return new JSDOM('<!DOCTYPE html><html><body><rich-textarea><div class="ql-editor" contenteditable="true"><p><br></p></div></rich-textarea></body></html>', {
      url: 'https://gemini.google.com/app'
    }).window.document;
  };
  const matrix = [['chatgpt', 'claude'], ['chatgpt', 'gemini'], ['claude', 'chatgpt'], ['claude', 'gemini'], ['gemini', 'chatgpt'], ['gemini', 'claude']];
