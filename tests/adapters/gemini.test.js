...
import { detectGeminiState } from '@/adapters/gemini/detector.js';
import { extractGeminiConversation } from '@/adapters/gemini/extractor.js';
import { injectGemini } from '@/adapters/gemini/injector.js';
describe('Gemini Adapter', () => {
  ...
  it('detects state, title, and conversationId from Gemini DOM', () => {
    ...
    const dom = new JSDOM(html, {
      url: 'https://gemini.google.com/app/test-gemini-session'
    });
  ...
  it('extracts conversation turns, code blocks, and markdown tables', async () => {
    ...
    expect(userMsg.content[0].text).toContain('Raft consensus');
    expect(assistantMsg.role).toBe('assistant');
    // Check code block
    const codeBlock = assistantMsg.content.find(b => b.type === 'code');
    expect(codeBlock).toBeDefined();
    expect(codeBlock.language).toBe('go');
    expect(codeBlock.code).toContain('package raft');

    // Check table extraction
    const tableBlock = assistantMsg.content.find(b => b.type === 'text' && b.text.includes('| Role | Responsibility |'));
    expect(tableBlock).toBeDefined();
    expect(tableBlock.text).toContain('| Leader | Handles all client requests');
  ...
  it('extracts multiturn conversations accurately', async () => {
    ...
    expect(t3.content[0].text).toContain('Python');
  ...
  it('handles incomplete history and provides honest warning', async () => {
    const dom = new JSDOM('<!DOCTYPE html><html><head><title>Gemini Long Chat</title></head><body><div class="chat-history"><user-query><div class="query-content">Only turn visible</div></user-query></div></body></html>', {
  ...
  it('returns failure when composer element is missing and timeout elapses', async () => {
    ...
    const {
      waitForGeminiInput
    } = await import('@/adapters/gemini/injector.js');
