...
import { detectChatGPTState } from '@/adapters/chatgpt/detector.js';
import { extractChatGPTConversation } from '@/adapters/chatgpt/extractor.js';
describe('ChatGPT Adapter', () => {
  ...
  it('extracts complete conversation with turns and code blocks from fixture', async () => {
    ...
    const dom = new JSDOM(html, {
      url: 'https://chatgpt.com/c/test-thread-id'
    });
    ...
    expect(t1.content[0].text).toContain('modular adapter system');
    expect(t2.role).toBe('assistant');
    const codeBlock2 = t2.content.find(b => b.type === 'code');
    expect(codeBlock2).toBeDefined();
    expect(codeBlock2.code).toContain('export interface Adapter');
    expect(codeBlock2.language).toBe('typescript');
    expect(t5.role).toBe('user');
    expect(t5.content[0].text).toContain('write the Registry class');
  });
  it('detects incomplete history when top turn is lazy loaded', async () => {
    ...
    const html = fs.readFileSync(fixturePath, 'utf-8');
    const dom = new JSDOM(html, {
      url: 'https://chatgpt.com/c/long-thread-id'
    });
  ...
  it('strips internal thought blocks and citations while preserving tables and code', async () => {
    ...
    const html = fs.readFileSync(fixturePath, 'utf-8');
    const dom = new JSDOM(html, {
      url: 'https://chatgpt.com/c/sharding-thread'
    });
    const result = await extractChatGPTConversation(dom.window.document, {
      scrollDelayMs: 0,
      networkTimeoutMs: 10
    ...
    expect(sqlCode.language).toBe('sql');
  });
});