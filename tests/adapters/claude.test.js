...
import { detectClaudeState } from '@/adapters/claude/detector.js';
import { extractClaudeConversation } from '@/adapters/claude/extractor.js';
import { injectClaude } from '@/adapters/claude/injector.js';
const loadFixture = name => fs.readFileSync(path.resolve(__dirname, '../fixtures', name), 'utf-8');
describe('Claude Adapter', () => {
  ...
  describe('Detection', () => {
    afterEach(() => {
      delete globalThis.PHERO_MOCK_IS_AT_TOP;
    });
    ...
    it('detects active conversation vs new-chat page', () => {
      ...
      const activeDom = new JSDOM(activeHtml, {
        url: 'https://claude.ai/chat/test-uuid'
      });
      ...
      const newDom = new JSDOM('<!DOCTYPE html><html><head><title>Claude</title></head><body></body></html>', {
    ...
    it('extracts conversation ID from URL path', () => {
      const dom = new JSDOM('<!DOCTYPE html><html><head><title>Claude</title></head><body></body></html>', {
    ...
    it('detects incomplete history when load-more button exists', () => {
      ...
      const dom = new JSDOM(html, {
        url: 'https://claude.ai/chat/incomplete-conv-id'
      });
      globalThis.PHERO_MOCK_IS_AT_TOP = false;
      const state = detectClaudeState(dom.window.document);
  ...
  describe('Extraction', () => {
    ...
    it('preserves TypeScript code blocks with language identifier', async () => {
      ...
      const codeBlock = assistantMsg.content.find(b => b.type === 'code' && b.language === 'typescript');
      expect(codeBlock).toBeDefined();
      expect(codeBlock.code).toContain('export interface Adapter');
      expect(codeBlock.code).toContain('AdapterRegistry');
    });
    it('extracts markdown table as text block with pipe-delimited content', async () => {
      ...
      const assistantMsg = result.conversation.messages[1];
      const tableBlock = assistantMsg.content.find(b => b.type === 'text' && b.text.includes('| Provider |'));
      expect(tableBlock).toBeDefined();
      expect(tableBlock.text).toContain('| ChatGPT |');
      expect(tableBlock.text).toContain('| Claude |');
    });
    it('strips UI noise: Copy code, Good response, Copied!', async () => {
      ...
      const allText = result.conversation.messages.flatMap(m => m.content.map(b => b.type === 'text' ? b.text : '')).join(' ');
    ...
    it('extracts artifact content from artifact containers', async () => {
      ...
      const hasArtifact = artifactMsg.content.some(b => b.type === 'code' && b.code.includes('CodeGenerator') || b.type === 'text' && b.text.includes('ARTIFACT'));
  ...
  describe('Injection', () => {
    it('injects continuation prompt into ProseMirror composer and verifies content', async () => {
      ...
      const prompt = 'You are continuing an ongoing conversation transferred from another AI assistant.\n\n=== INSTRUCTIONS ===\nContinue work on the adapter system.';
