import { deduplicateMessagesWithAudit, reindexMessages } from './deduplication.js';
import { getScrollMetrics, getVisibleTurnRange } from './scroll-helper.js';
import { Logger } from '../../shared/logger.js';
export class CaptureOrchestrator {
  static async executeCapture(doc, strategy, meta, options = {}) {
    ...
    let collectedMessages = strategy.captureCurrentVisibleMessages(doc);
    let reachedBeginning = strategy.isAtBeginning(doc, collectedMessages);
    let completenessState = reachedBeginning ? 'COMPLETE' : 'RECOVERING';
    ...
    const warning = !isComplete && finalMessages.length > 0 ? `Some earlier messages couldn't be retrieved. ${finalMessages.length} messages were captured.` : undefined;
    return {
      conversation: {
      ...
      completenessState,
      captureMethod: 'DOM_VIRTUALIZATION',
      isComplete,
