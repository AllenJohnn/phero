...
export function getBudgetConfigForProvider(provider) {
...
export function partitionConversation(messages, config = DEFAULT_BUDGET_CONFIG) {
  ...
  const extractedUnresolvedIssues = [];
    ...
    for (const block of msg.content) {
      if (block.type === 'code' && block.code.trim().length > 0) {
        ...
        for (const line of lines) {
          ...
          if (/^(require(ment)?|must|constraint|rule|goal|instruction|note|spec|acceptance criteria):/i.test(trimmed) || /^-\s*(must|shall|should not|do not|never|always)\b/i.test(trimmed)) {
          ...
          } else if (/^(decision|we decided|agreed on|chosen approach|architecture choice|design):/i.test(trimmed) || /\b(let's go with|we'll use|chosen to use|settled on)\b/i.test(trimmed)) {
            const clean = trimmed.replace(/^[-*•]\s*/, '').trim();
            const key = clean.toLowerCase();
          ...
          } else if (/^(bug|error|issue|unresolved|todo|blocker|failure):/i.test(trimmed) || /\b(fix needed|still failing|broken|exception encountered)\b/i.test(trimmed)) {
            const clean = trimmed.replace(/^[-*•]\s*/, '').trim();
