...
export class InjectionCoordinator {
  static async checkAndPerformInjection(destinationProvider) {
    try {
      Logger.info('Checking for pending handoff', {
      ...
      while ((!result.success || !result.verified) && retries < 2) {
        ...
        try {
          ...
          Logger.warn(`Retry ${retries} failed while waiting for editor`, {
  ...
  static showStatusBanner(text, type) {
    this.removeBanner();
