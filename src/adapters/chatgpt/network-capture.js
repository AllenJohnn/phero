...
let cachedConversationData = new Map();
export function installNetworkCaptureListener(doc) {
  doc.addEventListener('__phero_chatgpt_log__', e => {
    Logger.info('[MAIN WORLD] ' + e.detail);
  });
  doc.addEventListener(PHERO_NETWORK_EVENT, e => {
    let data = e.detail;
...
export function parseConversationMapping(mapping, currentNode) {
  const path = [];
...
export async function attemptNetworkCapture(url) {
  ...
  if (!data) {
    ...
    document.dispatchEvent(new CustomEvent('__phero_request_conversation_data__', {
      detail: uuid
    }));
    for (let i = 0; i < 6; i++) {
      await new Promise(r => setTimeout(r, 500));
