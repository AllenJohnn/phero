import { getVisibleTurnRange } from '../../core/capture/scroll-helper.js';
...
function getStructuralIdentity(el) {
  if (!el) return 'none';
...
export function startManualScrollDiagnostics(doc) {
  ...
  let beforeStateElements = null;
  let beforeStateGlobal = null;
  let lastIdentifiedContainer = null;
  ...
  const logTransition = () => {
    ...
    if (beforeStateGlobal.windowScrollY !== afterGlobal.windowScrollY || beforeStateGlobal.docElementScrollTop !== afterGlobal.docElementScrollTop || beforeStateGlobal.scrollingElementScrollTop !== afterGlobal.scrollingElementScrollTop) {
  ...
  if (win) {
    win.addEventListener('wheel', handleWheelCapture, {
      capture: true,
      passive: true
    });
  } else {
  ...
  globalTeardown = () => {
    if (win) {
      win.removeEventListener('wheel', handleWheelCapture, {
        capture: true
      });
    } else {
      doc.removeEventListener('wheel', handleWheelCapture, {
        capture: true
      });
    }
    observer.disconnect();
