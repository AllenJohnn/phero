import { detectChatGPTState } from './detector.js';
import { Logger } from '../../shared/logger.js';
export function extractContentBlocksFromElement(element) {
  const blocks = [];
  ...
  for (const child of children) {
    ...
    if (child.classList.contains('thought-details') || child.getAttribute('data-testid') === 'thought-block' || child.tagName === 'DETAILS') {
      continue;
    }
    const clone = child.cloneNode(true);
    clone.querySelectorAll('button, svg, [role="button"], .sr-only, details, [data-testid="thought-block"], .citation, sup').forEach(el => el.remove());
...
import { ChatGPTCaptureStrategy } from './capture.js';
import { CaptureOrchestrator } from '../../core/capture/orchestrator.js';
import { attemptNetworkCapture } from './network-capture.js';
export async function extractChatGPTConversation(doc, options = {}) {
  const state = detectChatGPTState(doc);
  Logger.info('Extracting ChatGPT conversation', {
