...
(function () {
  function sendLog(msg) {
    console.log(msg);
    document.dispatchEvent(new CustomEvent(PHERO_LOG_EVENT, {
      detail: msg
    }));
  }
  ...
  function emitData(data) {
    if (data && data.mapping && data.conversation_id && data.current_node) {
      ...
      const event = new CustomEvent(PHERO_NETWORK_EVENT, {
        detail: serialized
      });
  ...
  const loadCheckInterval = setInterval(() => {
    ...
    checkNextData();
    loadCheckCount++;
    if (loadCheckCount > 20 || window.__remixContext) {
  ...
  function getConversationUuidFromUrl(href) {
