...
export function detectClaudeState(doc) {
  ...
  let title = '';
  const titleEl = doc.querySelector('button[data-testid="chat-title-button"], div[data-testid="chat-title"], h1.chat-title, [data-testid="conversation-title"]');
  if (titleEl && titleEl.textContent?.trim()) {
    title = titleEl.textContent.trim();
  }
  if (!title && doc.title) {
    title = doc.title.replace(/\s*[-–—|]\s*Claude\s*$/i, '').replace(/^Claude\s*[-–—|]\s*/i, '').trim();
  }
