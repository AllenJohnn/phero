export function isGeminiUrl(url) {
  ...
  return host === 'gemini.google.com' || host.endsWith('.gemini.google.com') || host === 'bard.google.com' || host.endsWith('.bard.google.com');
}
export function detectGeminiState(doc) {
  ...
  const genericTurns = doc.querySelectorAll('div[data-test-id="conversation-turn"], conversation-turn');
  let messageCount;
  if (genericTurns.length > 0) {
    messageCount = genericTurns.length;
  } else {
    const userTurns = doc.querySelectorAll('user-query, .user-query-container, div[data-test-id="user-query"]');
    const assistantTurns = doc.querySelectorAll('model-response, .response-container, div[data-test-id="model-response"]');
  ...
  if (!title) {
    const titleEl = doc.querySelector('div[data-test-id="conversation-title"], .conversation-title, h1.title, .chat-title');
    if (titleEl && titleEl.textContent?.trim()) {
      title = titleEl.textContent.trim();
    }
  }
  if (!title && doc.title) {
    title = doc.title.replace(/\s*[-–—|]\s*Gemini\s*$/i, '').replace(/^Gemini\s*[-–—|]\s*/i, '').trim();
  }
