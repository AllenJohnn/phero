...
import { PheroLogo, SpinnerIcon, CheckIcon, AlertIcon, CopyIcon } from '../icons/index.jsx';
...
export const FloatingPill = ({
  ...
  const handleStartHandoff = async (destination, forceContinue = false) => {
    try {
      setIsOpen(false);
  ...
  return <div className="fixed bottom-5 right-5 z-[999999] font-sans antialiased select-none">
      {}
      {(status === 'completeness_warning' || status === 'streaming_warning') && <div className="absolute bottom-11 right-0 w-72 rounded-xl border border-[#D97706]/40 bg-[#09090B] p-3.5 text-[#F4F4F6] shadow-2xl backdrop-blur-md">
      ...
      {}
      {isOpen && status === 'idle' && <QuickSwitcher sourceProvider={sourceProvider} onSelectDestination={dest => handleStartHandoff(dest)} onClose={() => setIsOpen(false)} />}
      ...
      if (status === 'idle') {
    ...
    }} disabled={status === 'extracting' || status === 'building_context' || status === 'opening_destination'} className={`group flex items-center gap-2 px-3 py-1.5 rounded-full border shadow-lg transition-all duration-150 cursor-pointer ${status === 'idle' ? 'bg-[#09090B]/90 hover:bg-[#111113] border-[#232326] hover:border-[#36363A] text-[#EDEDEF] hover:text-white' : status === 'ready' ? 'bg-[#064E3B]/90 border-[#059669]/60 text-[#A7F3D0]' : status === 'failed' ? 'bg-[#881337]/90 border-[#E11D48]/60 text-[#FECDD3]' : 'bg-[#111113] border-[#232326] text-[#A1A1AA]'}`}>
        <div className="flex items-center justify-center">
          {status === 'idle' && <PheroLogo size={14} className="text-[#3B82F6] group-hover:scale-105 transition-transform" />}
          {(status === 'extracting' || status === 'building_context' || status === 'opening_destination') && <SpinnerIcon size={13} className="text-[#3B82F6]" />}
        ...
        {status === 'idle' && <span className="text-[11px] text-[#52525B] group-hover:text-[#A1A1AA] transition-colors">
            ↗
          </span>}

        {status === 'failed' && preparedPayload && <div onClick={e => {
        e.stopPropagation();
        handleCopyPrompt();
